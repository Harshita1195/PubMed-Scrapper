...
# keyword = str(input('Enter the keyword or title: '))
# num = int(input('Enter the number of articles: '))

def scrapper(num, keyword):
    # replacing the spaces with '+', based on pubmed search engine.
    keyword_search = keyword.replace(' ', '+')

    # url
    url=f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax={num}&sort=relevance&term={keyword_search}"
    ...
    df.columns = ['Authors', 'Title', 'Journal', 'Volume', 'Date', 'PubmedID', 'DOI', 'Abstract']
    file_name = 'static/articles.csv'
